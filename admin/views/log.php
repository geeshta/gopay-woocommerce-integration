...
$log_data = $wpdb->get_results(sprintf("SELECT * FROM %s%s ORDER BY created_at DESC LIMIT %d,%d",
    $wpdb->prefix, WOOCOMMERCE_GOPAY_LOG_TABLE_NAME, $page_pagination, $results_per_page));
    ...
    <div class="woocommerce-gopay-menu">
        <table>
            ...
            foreach ($log_data as $_ => $log) {
                ...
                $gw_url = (!empty($log_decoded) && property_exists($log_decoded->json, "gw_url")) ? $log_decoded->json->gw_url : "#";
        ...
        <nav>
            <ul class="woocommerce-gopay-menu-pagination">
                ...
                for ($page = 1; $page <= $number_of_pages; $page++) {
                    echo '<li class="woocommerce-gopay-menu-' . ($pagenum == $page ? 'active' : 'inactive') . '"><a href = "'
